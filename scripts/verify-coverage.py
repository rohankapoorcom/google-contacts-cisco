...
def calculate_module_coverage(files: Dict[str, Dict]) -> Dict[str, Dict]:
    ...
    for module_data in modules.values():
        if module_data["total"] > 0:
            module_data["percent"] = (module_data["covered"] / module_data["total"]) * 100
        else:
...
def print_module_breakdown(modules: Dict[str, Dict], threshold: float):
    ...
    for module, data in sorted_modules:
        ...
        print(f"  {marker} {color}{percent:6.2f}%{NC}  {module:40s}  "
              f"({covered:4d}/{total:4d} lines, {file_count} files) {critical_marker}")


def identify_gaps(modules: Dict[str, Dict], threshold: float) -> List[Tuple[str, float]]:
...
def print_coverage_gaps(gaps: List[Tuple[str, float]], threshold: float):
    ...
    for module, percent in gaps:
        ...
        if is_critical:
            print(f"  {RED}✗ {module:40s}  {percent:6.2f}%  "
                  f"(need {diff:+.2f}%) [CRITICAL]{NC}")
        else:
            print(f"  {YELLOW}⚠ {module:40s}  {percent:6.2f}%  "
                  f"(need {diff:+.2f}%){NC}")
...
def print_recommendations(gaps: List[Tuple[str, float]], modules: Dict[str, Dict]):
    ...
    critical_gaps = [
        (module, percent) for module, percent in gaps
...
def main():
    ...
    if args.critical_only:
        modules = {
            name: data for name, data in all_modules.items()
    ...
    if overall_percent < fail_threshold:
        print(f"{RED}✗ Coverage {overall_percent:.2f}% is below threshold "
