...
class TestOAuthFlowIntegration:
    ...
    @patch("google_contacts_cisco.auth.oauth.get_credentials")
    def test_oauth_status_without_credentials(self, mock_get_creds, integration_client):
...
class TestOAuthTokenManagement:
    ...
    @patch("builtins.open", new_callable=mock_open)
    def test_save_credentials_to_file(
        self, mock_file, mock_get_token_path, temp_token_file
    ):
