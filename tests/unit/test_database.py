...
from google_contacts_cisco.models.db_utils import (
    check_connection,
    create_tables,
    drop_tables,
)
...
def test_contact_with_phone_numbers(db_session):
    ...
    contact = Contact(resource_name="people/12345", display_name="John Doe")
    db_session.add(contact)
    db_session.flush()
...
def test_contact_multiple_phone_numbers(db_session):
    ...
    contact = Contact(resource_name="people/12345", display_name="Jane Smith")
    db_session.add(contact)
    db_session.flush()
...
def test_contact_soft_delete(db_session):
    ...
    contact = Contact(
        resource_name="people/12345", display_name="John Doe", deleted=False
    )
    db_session.add(contact)
    db_session.commit()
...
def test_sync_state(db_session):
    ...
    sync_state = SyncState(sync_token="token123", sync_status=SyncStatus.IDLE)
...
def test_sync_state_with_error(db_session):
    ...
    sync_state = SyncState(
        sync_status=SyncStatus.ERROR, error_message="Connection timeout"
    )
    db_session.add(sync_state)
    db_session.commit()
...
def test_contact_unique_resource_name(db_session):
    ...
    contact1 = Contact(resource_name="people/12345", display_name="John Doe")
    db_session.add(contact1)
    db_session.commit()

    # Try to create another contact with same resource_name
    contact2 = Contact(resource_name="people/12345", display_name="Jane Doe")
...
def test_phone_number_required_fields(db_session):
    ...
    with pytest.raises(Exception):
        phone = PhoneNumber(contact_id=contact.id, display_value="(123) 456-7890")
        db_session.add(phone)
        db_session.commit()
...
def test_phone_number_foreign_key(db_session):
    ...
    This test verifies the relationship exists but doesn't enforce
...
def test_contact_query_by_display_name(db_session):
    ...
    contact1 = Contact(resource_name="people/1", display_name="Alice")
    contact2 = Contact(resource_name="people/2", display_name="Bob")
...
def test_phone_number_query_by_value(db_session):
    ...
    contact = Contact(resource_name="people/1", display_name="John Doe")
    db_session.add(contact)
    db_session.flush()
...
def test_phone_number_primary_default(db_session):
    ...
    phone = PhoneNumber(
        contact_id=contact.id, value="1234567890", display_value="(123) 456-7890"
    )
    db_session.add(phone)
    db_session.commit()
